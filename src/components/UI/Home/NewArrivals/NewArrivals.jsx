...
export const NewArrivals = () => {
    ...
    const newArrivalProducts = () => {
        if (prods.length > 0){
            const docs = prods.filter(product => product.category === "mobile" || product.category === "wireless");
  ...
  return(
      <>
          <div className="container flex flex-col place-content-center mx-auto">
              <h2 className="text-center pt-16 text-2xl md:text-3xl font-bold text-blue-gray-900">New Arrivals</h2>
                          ...
                          <motion.div key={id} whileHover={{scale: 0.9}}>
                              <Card className="w-full shadow-none static">
                                  <CardHeader floated={false} className=" h-56 shadow-none static">
                                      <img src={item.imgUrl} alt={`trending-product-${id}`} className="h-full"/>
                                  </CardHeader>
