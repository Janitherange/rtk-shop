...
export const PopularInCategory = () => {
    ...
    const popularProducts = () => {
      setProducts(prods.filter(product=> product.category === "watch"));
    }
  ...
  return(
      <>
          <div className="container flex flex-col place-content-center mx-auto">
              <h2 className="text-center text-2xl md:text-3xl font-bold text-blue-gray-900">Popular in Category</h2>
                      ...
                      products && products.map((item, id)=>(
                          <motion.div key={id} whileHover={{scale: 0.9}}>
                              <Card className="w-full shadow-none">
                                  <CardHeader floated={false} className=" h-28 md:h-40 lg:h-56 shadow-none flex justify-center">
                                      <img src={item.imgUrl} alt={`popular-category-product-${id}`} className="bg-cover h-full"/>
                                  </CardHeader>
                                  <CardBody className="px-6 py-2 ">
                                      <Typography variant="h3" className="text-blue-gray-900 text-sm lg:text-xl">{item.productName}</Typography>
                                      <Typography variant="paragraph" className="text-blue-gray-700 font-extralight text-xs">{item.category}</Typography>
                                  </CardBody>
